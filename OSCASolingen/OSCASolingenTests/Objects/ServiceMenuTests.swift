...
///  | pygmentize -g
///  ```
class ServiceMenuTests: OSCAParseClassObjectTestSuite<ServiceMenu> {
  override open func makeSpecificObject() -> ServiceMenu? {
    nil
  ...
  /// The file name has to match with the test class name: `ServiceMenuTests.json`
  override func testJSONDataAvailable() throws -> Void {
    try super.testJSONDataAvailable()
  }// end override func testJSONDataAvailable
#if DEBUG
  /// Is it possible to deserialize `JSON` scheme example data to an array  `ServiceMenu` 's?
  override func testDecodingJSONData() throws -> Void {
    try super.testDecodingJSONData()
  }// end override func testDecodingJSONData
#endif
  /// is it possible to fetch up to 1000 `ServiceMenu` objects in an array from network?
