...
///  https://parse-dev.solingen.de/classes/PushTemplate \
///  | python3 -m json.tool
///  | pygmentize -g
///  ```
class PushTemplateTests: OSCAParseClassObjectTestSuite<PushTemplate> {
  override open func makeSpecificObject() -> PushTemplate? {
    nil
  ...
  /// The file name has to match with the test class name: `PushTemplateTests.json`
  override func testJSONDataAvailable() throws -> Void {
    try super.testJSONDataAvailable()
  }// end override func testJSONDataAvailable
#if DEBUG
  /// Is it possible to deserialize `JSON` scheme example data to an array  `PushTemplate` 's?
  override func testDecodingJSONData() throws -> Void {
    try super.testDecodingJSONData()
  }// end override func testDecodingJSONData
#endif
  /// is it possible to fetch up to 1000 `PushTemplate` objects in an array from network?
